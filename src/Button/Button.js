...
const Button = React.forwardRef(
  (
    ...
    const buttonTextClasses = classnames(`${cssNamespace}-button__text`, textClassName);

    const ariaDisabled = props['aria-disabled'];
    const disabledInAnyWay = disabled || ariaDisabled;

    //if button is disabled and focusable, remove onClick handler
    const onClickHandler = allowFocusOnDisable && disabledInAnyWay ? () => {} : onClick;
    ...
    const renderIcon = () => {
      const content = glyph ? <Icon {...iconProps} ariaHidden glyph={glyph} /> : null;
...
const validateStateTransitionMessage = (props, propName) => {
  if (
    props.allowFocusOnDisable &&
    (props.disabled !== null || props['aria-disabled']) &&
    !props[propName]
  ) {
    return new Error(`
